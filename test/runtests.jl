...
@testset "FunctionIndices" begin
    ...
    @testset "not for outbound index" begin
        @test A[not(0)] == A[not(20)] == OA[not(0)] == OA[not(20)] == vec(A) == vec(OA)
        @test A[not(0:13)] == A[not(1:12)] == OA[not(0:13)] == OA[not(1:12)]
        @test A[not(-1:1)] == A[not(1:1)] == OA[not(-1:1)] == OA[not(1:1)]
        @test A[not(0:2), not(0)] == A[not(1:2), not(5)] == OA[not(1:3), not(-1)] == OA[not(1:3), not(4)] == A[3:3, 1:4] == OA[4:4, 0:3]
    end
